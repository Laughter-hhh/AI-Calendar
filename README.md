...
- **离线使用**：打开过的日程按账号缓存到本地，断网时可继续切换查看（显示"离线模式"提示）；HTTPS/localhost 下同时注册 Service Worker，冷启动会显示离线回退页
- **混合识别**：联网时用 AI 大模型解析，离线/未配置时自动用本地规则解析器（信息缺失会追问）
- **双界面**：单日可在紧凑事项与时间线间切换；周视图显示 7 天时间块，重叠任务自动并排；移动端支持右侧左滑返回上一级
- **安全 ICS 导入**：支持 Google Calendar / Apple 日历 / Outlook 导出的 `.ics`，先预览再追加，重复事件跳过且不覆盖原日程
- **无时间待办**：说"待办：明天交报告"或点"＋待办"即可添加，不再强制要时间
- **待办自动识别**：只写"交报告"（无日期无时间）会自动变成当天待办
- **截止日期提醒**：说"8月31号前完成实验报告"会自动生成提前 7/3/1 天和当天的提醒待办
- **中文数字日期**："八月三十一之前完成年度审核"同样识别
- **彩色日程**：新建日程自动随机分配非灰色颜色，时间轴一眼区分任务
