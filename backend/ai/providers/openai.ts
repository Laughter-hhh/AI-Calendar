...
const NORMALIZE_SYSTEM = [
  ...
  "- 重复：每天 <时间或时间段> <事项>、每周X <时间或时间段> <事项>、每两周X（也可写每两周周X） <时间或时间段> <事项> 或 每月N日 <时间或时间段> <事项>",
  "- 非连续周次：从<日期>开始第1、3、6、7周 <时间或时间段> <事项>；按月周次：每月第1、3周周X <时间或时间段> <事项>；排除规则：每周X，除了第2周 <时间或时间段> <事项>，或每周X，除了9月10日所在的那周 <时间或时间段> <事项>（必须保留第几周/排除语义）",
  ...
  "- 多项独立日程：同一天的多个时间事项（例：2026-09-10 15:00 开会，18:00 吃饭）和不同日期事项（例：2026-09-10 15:00 开会，2026-09-11 18:00 写报告）必须分别保留为多个事项，不得合并标题或覆盖日期。",
  ...
  "3.4. ‘每隔两周/每两周一次’必须标准化为‘每两周’，不得改写成每周或一次性事项；",
  "3.5. 指定第几周、多个不连续周次、‘除了第几周’或‘除了某日期所在的那周’必须原样保留这些规则，不得改写成普通‘每周’；",
