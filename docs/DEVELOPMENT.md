...
- **已完成**：注册/登录、AI 自然语言创建日程（连续 N 天）、信息追问、语音输入、单日/未来7天/月三种视图、搜索、事件详情与备注、增强编辑（日期/结束时间/备注/颜色）、完整重复事件（每天/每周/每月 + 截止日期 + 仅删本日）、AI 自然语言修改/删除日程、ICS 导入导出、事件颜色分类、今日下一项提示、手机 App（WebView 壳 + 独立 APK 网页直下）、CI 自动部署、数据库备份脚本、冒烟测试（60 项+）与页面渲染测试（28 项）、**笔记本**（不确定时间的事：记录/勾选/编辑/删除/一键转为日程）
- **当前版本标签**：`v4.5.9`
...
│   ├── src/lib/                           # 数据层（db/auth/events/notes/date）
...
**notes**（笔记本，v4.4.0 新增）：`id, user_id(按用户隔离), text(≤500 字), done(0/1), created_at`

> 笔记本条目**不是日程**：无日期无时间，不参与日历查询、不导出（ICS）；时间确定后在前端"转为日程"并自动移出。
...
| GET | /api/notes | 笔记本列表（未完成在前） | 登录 |
| POST | /api/notes | 新增笔记本条目 | 登录 |
| PATCH | /api/notes/:id | 修改笔记本条目（文字/完成状态） | 登录（本人） |
| DELETE | /api/notes/:id | 删除笔记本条目 | 登录（本人） |
...
- 缺失追问：缺标题/日期 → missing 数组；**有日期无时间 → 全天待办（不追问）**（v4.3.7 起）
...
| v3.4.0 | 2026-08-26 | iOS 构建配置与分发流程就绪（ATS、TestFlight 文档） | `v3.4.0` |
  ...
  | v4.3.6 | 2026-08-29 | 追问提示回显已识别信息，并提示“无时间/待办”可创建全天待办 | `v4.3.6` |
  | v4.3.7 | 2026-08-29 | 有日期无时间 → 默认全天待办（不再追问时间），规则同步到 skill/帮助页/测试 | `v4.3.7` |
  | v4.3.8 | 2026-08-29 | 修复"X月Y日完成事项"被误判为完成操作/截止提醒（放行创建为全天待办） | `v4.3.8` |
  | v4.4.0 | 2026-08-29 | 笔记本：记录"不确定什么时候做"的事（增删改查、勾选完成、一键转为日程） | `v4.4.0` |
  | v4.5.0 | 2026-08-31 | 安全 ICS 导入、可编辑起止时间、重叠并排、单日时间线、云端测试与失败回滚 | `v4.5.0` |
  | v4.5.1 | 2026-08-31 | 修复单日时间线顶部留白；加入安全 Service Worker 应用壳、离线回退页和网络恢复自动刷新 | `v4.5.1` |
  | v4.5.2 | 2026-08-31 | 移动端笔记本返回日历复用历史页面，空时间线提示遮挡网格线，客户端导航减少整页加载 | `v4.5.2` |
  | v4.5.3 | 2026-08-31 | 全局移动端右侧左滑返回上一级，表单控件防误触，更新应用壳缓存版本 | `v4.5.3` |
  | v4.5.4 | 2026-09-01 | 修复本周多日期被误判为每周重复；笔记页加载与离线导航超时优化；清理未引用模板资源 | `v4.5.4` |
  | v4.5.5 | 2026-09-01 | 修复旧 SQLite 数据库迁移不完整导致日程保存 500；增加本周多日期保存回归 | `v4.5.5` |
  | v4.5.6 | 2026-09-01 | 新增可查看缓存日程/笔记的移动端离线界面；首屏缓存账号隔离；CI 发布前统一生成并校验测试数据 | `v4.5.6` |
  | v4.5.7 | 2026-09-01 | 修复“具体日期之后每周多个星期”复合日程识别：拆分一次性事件与独立 weekly 事件，并从前一段日期之后起算 | `v4.5.7` |
  | v4.5.8 | 2026-09-01 | 修复时间段日程在缺少日期追问后丢失结束时间的问题，确保 15:00–18:00 保存后保持原区间 | `v4.5.8` |
  | v4.5.9 | 2026-09-02 | 固定 AI 标准时间段与多星期格式；本地解析支持 HH:mm-HH:mm，并校正 AI 将每周误判为本周的问题 | `v4.5.9` |
