...
```
用户输入
   ├─ 联网 + 已配 Key ──► 大模型理解语义 → 改写成标准句式
   │                        └─ 本地固定程序按标准句式生成日程（截止提醒/连续/重复/待办/追问）
   └─ 离线 / 未配 Key / 模型失败 ──► 本地规则解析器（同一套固定程序）
                                       └─ 自动分割：连续N天/重复/日期/时间/标题
                                          缺失信息追问
```

每次解析返回 `provider` 字段标明所用模式，无需手动切换。**确定性逻辑（截止 7/3/1 天+当天提醒、连续 N 天展开等）始终由本地规则生成，模型只做理解与标准化。**

### AI 标准化输出格式（v4.5.9）

- 单时间统一为 `HH:mm`，时间段统一为 `HH:mm-HH:mm`，禁止输出“到/至”替代连字符。
- 多星期重复统一为“每周四和周五”这类写法，保留在同一行，由本地解析器拆成独立 weekly 日程。
- 复合安排统一用“日期 时间段 事项；之后每周四和周五 时间段 事项”，前后两段分别生成一次性和重复日程。
- 模型输出违反明确的时间段或每周语义时，服务层自动回到原句进行确定性解析。
